...
//  ********** Library C:\Users\smarth\dart\DartGraph\DartGraph **************
// ********** Code for DartGraph **************
function DartGraph() {
  this.colors = ["#f00", "#0f0", "#00f", "#0ff", "#ff0", "#f0f"];
}
DartGraph.bar = function(CanvasID) {
  return new Bar(CanvasID, "Bar Graph");
}
DartGraph.line = function(CanvasID) {
  return new Line(CanvasID, "Line Graph");
}
DartGraph.pie = function(CanvasID) {
  return new Pie(CanvasID, "Pie Graph");
}
// ********** Code for Bar **************
$inherits(Bar, DartGraph);
function Bar(CanvasID, Title) {
  DartGraph.call(this);
  this.X_Title = "";
  this.Y_Title = "";
...
Bar.prototype.draw = function() {
  var GraphCanvas = get$$document().query(("#" + this.CanvasID));
...
// ********** Code for Line **************
$inherits(Line, DartGraph);
function Line(CanvasID, Title) {
  DartGraph.call(this);
  this.CanvasID = CanvasID;
  this.GraphTitle = Title;
  this.X_Title = "";
...
// ********** Code for Pie **************
$inherits(Pie, DartGraph);
function Pie(CanvasID, Title) {
  DartGraph.call(this);
  this.CanvasID = CanvasID;
  this.GraphTitle = Title;
}
...
function find_max(data) {
  var max = data.$index((0));
  var i = (0);
  for (i = (0);
   i < data.get$length(); ++i) {
    if ($gt$(data.$index(i), max)) {
      max = data.$index(i);
    }
  }
  return max;
}
function find_max_graph(max) {
  var n = max;
  var count = (0);
  while (n != (0)) {
    n = (n / (10)).toInt();
  ...
  var mult = Math.pow((10), count - (1));
  var maxceil = ((max / mult).toInt() + (1)) * mult;
  if ((maxceil - (0.5) * mult) > max) return maxceil - ((0.5) * mult);
  else return maxceil;
}
function plotbars(Data, Labels, Colors, ctx, width, height) {
  var avail_width = (0.675) * width;
  var avail_height = (0.85) * height;
  var max = find_max(Data);
  var max_graph = find_max_graph(max);
  var total_bars = Data.get$length();
  var bar_width = avail_width / total_bars;
  var scale_factor = (avail_height / max_graph);
  var i;
  for (i = (0);
   i < Data.get$length(); ++i) {
    var bar_height = $mul$(scale_factor, Data.$index(i));
    ctx.fillStyle = Colors.$index(i);
    ctx.textAlign = "center";
    ctx.strokeText(Labels.$index(i), ((0.25) * width) + (i * bar_width) + (0.375) * bar_width, (0.91) * height - bar_height);
    ctx.fillRect(((0.25) * width) + (i * bar_width), (0.93) * height - bar_height - (2), (0.75) * bar_width, bar_height);
  }
}
function plotscale(Data, ctx, width, height) {
  var max = find_max(Data);
  var max_graph = find_max_graph(max);
  var division = max_graph / (10);
